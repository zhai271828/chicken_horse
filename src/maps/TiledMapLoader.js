...
export class TiledMapLoader {
    ...
    getCoins(placedObstacles = []) {
        ...
        // Collect all coin objects first, then randomly pick one as rainbow
        const coinObjects = [];
        for (const layer of this.mapData.layers) {
        ...
        const rainbowIndex = coinObjects.length > 0 ? Math.floor(Math.random() * coinObjects.length) : -1;

        const coins = [];
        for (let ci = 0; ci < coinObjects.length; ci++) {
            const obj = coinObjects[ci];
            ...
            const isRainbow = ci === rainbowIndex;
            ...
            coins.push(
                new Coin(
                    ...
                    cx,
                    cy,
                    isRainbow ? GameConfig.RAINBOW_COIN_VALUE : GameConfig.COIN_VALUE,
                    this.coinSprite,
                    visualOffsetX,
                    isRainbow,
                ),

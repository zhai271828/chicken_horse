...
export class BuildState extends State {
    ...
    enter() {
        ...
        // Do NOT regenerate map here — map only changes when advancing at 100 points
        // (handled by ResultsState → advanceMap → MAPMENU → MapMenuState)
        ...
        // Keep existing obstacles from previous rounds — they persist until map changes
        // Players can add NEW traps on top of existing ones
        this._currentTurn = 0;
    ...
    _createObstacle(type, x, y) {
        ...
        if (obs) {
            ...
            obs._placedBy = this._currentTurn;
            this.ctx.scoreManager?.registerTrap(obs, this._currentTurn);
            this.ctx.scoreManager?.recordTrapPlacement?.(this._currentTurn);
        }
        return obs;
    }
}

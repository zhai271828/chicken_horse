...
function _drawFrame(player, p, frames, indexKey, flipOnLeft) {
    if (!Array.isArray(frames) || frames.length === 0) {
        _drawFallbackFrame(player, p, flipOnLeft);
        return;
    }
...
function _drawFallbackFrame(player, p, flipOnLeft) {
    const fallback =
        player.framesArr?.[player.animConfig?.IDLE?.[0] ?? 0] ||
        player.framesArr?.[0];
    if (!fallback) return;
    ...
    if (flipOnLeft && !player.facingRight) {
        p.translate(player.x + player.w, player.y);
        p.scale(-1, 1);
        p.image(fallback, 0, 0);
    } else {
        p.image(fallback, player.x, player.y);
    }
    p.pop();
}

...
export class Slime extends Obstacle {
    constructor(p, x, y) {
        super(p, x, y);
        this._affectedPlayers = new Map(); // playerNo -> {player, remaining}
    }
    get isSolid() { return false; }
    get isHazard() { return false; }

    update(deltaTime) {
        for (const [key, entry] of this._affectedPlayers) {
            const next = entry.remaining - deltaTime;
            if (next <= 0) {
                // Reset player properties when slime effect expires
                const player = entry.player;
                if (player) {
                    player.jumpMultiplier = 1.0;
                    player.speedMultiplier = 1.0;
                    player.slideMode = false;
                }
    ...
    preEffect(player) {
        ...
        if (inSlime) {
            ...
            this._affectedPlayers.set(player.playerNo, { player, remaining: GameConfig.SLIME_DURATION_MS });
        } else {
            const entry = this._affectedPlayers.get(player.playerNo);

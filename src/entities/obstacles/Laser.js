...
export class Laser extends Obstacle {
        ...
        switch (this._state) {
            ...
            case 'aiming':
                ...
                if (this._stateTimer >= 2000) {
                    ...
                    const dist = Math.sqrt(dx * dx + dy * dy);
                    const beamDist = Math.min(dist, maxDist);
                    const ratio = beamDist / dist;
                    this._beamEndX = sx + dx * ratio;
                    this._beamEndY = sy + dy * ratio;

                    // Check for solid tiles along beam path and truncate
                    if (MAP) {
                        const steps = 20;
                        for (let i = 1; i <= steps; i++) {
                            const t = i / steps;
                            const bx = sx + (this._beamEndX - sx) * t;
                            const by = sy + (this._beamEndY - sy) * t;
    ...
    draw() {
        ...
        if (this._state === 'aiming') {
            // Dotted aiming line — truncated by solid tiles
            p.stroke(255, 50, 50, 100 + Math.sin(this._age * 0.01) * 50);
            p.strokeWeight(1);
            let endX = this._targetX;
            let endY = this._targetY;
            const MAP = this._ctx?.tiledMap?.MAP;
            if (MAP) {
                const steps = 20;
                for (let i = 1; i <= steps; i++) {
                    const t = i / steps;
                    const bx = cx + (this._targetX - cx) * t;
                    const by = cy + (this._targetY - cy) * t;
                    const tx = Math.floor(bx / T);
                    const ty = Math.floor(by / T);
                    if (ty >= 0 && ty < MAP.length && tx >= 0 && tx < (MAP[ty]?.length ?? 0)) {
                        if (this._isSolidTile(MAP[ty][tx])) {

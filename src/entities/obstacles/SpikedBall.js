...
export class SpikedBall extends Obstacle {
    ...
    update(deltaTime, gameWidth, gameHeight, MAP) {
        ...
        const moveX = this._vx * this._dir * dtFactor;
        this.x += moveX;
        ...
        // Bounce off solid tiles ahead with random speed
        if (MAP) {
            const aheadX = this._dir > 0 ? this.x + this.w + 1 : this.x - 1;
            const midY = this.y + this.h / 2;
            const atx = Math.floor(aheadX / T);
            const aty = Math.floor(midY / T);

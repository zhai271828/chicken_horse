...
export class Player {
    ...
    update(
        ...
        const spikeResult = checkSpikeCollision(this, this.p, obstacles, MAP);
        if (spikeResult) {
            // spikeResult is either true (map spike) or the obstacle that caused it
            const ownerNo = (spikeResult !== true && spikeResult._obstacleId && respawnManager.scoreManager)
                ? respawnManager.scoreManager.getTrapOwner(spikeResult)
                : null;
            const trapType = spikeResult !== true ? spikeResult.type || null : null;
            respawnManager.triggerDeath(this, DeathReason.TRAP, ownerNo, trapType);
            return;
        }

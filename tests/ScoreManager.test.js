...
describe('ScoreManager', () => {
    ...
    it('tracks trap types and rolling death windows', () => {
        manager.recordDeath(players[0], 1, 'LASER');
        manager.recordDeath(players[0], 1, 'FLAME');
        manager.recordDeath(players[0], 1, 'LASER');

        const score = manager.getScore(players[0]);
        expect(score.trapTypesKilled.size).toBe(2);
        expect(score.deathsInLastMinute).toBe(3);
    });
    ...
    it('uses current map points for MVP title progress', () => {
        manager.points.set(0, 30);
        manager.points.set(1, 12);
        manager.totalPoints.set(0, 30);
        manager.totalPoints.set(1, 999);

        const titleHits = manager.recordRoundTitleProgress();

        expect(titleHits.get(0)?.has('mvp')).toBe(true);
        expect(titleHits.get(1)?.has('mvp')).not.toBe(true);
    });

    it('uses current map points in map-advance title summaries', () => {
        manager.points.set(0, 45);
        manager.points.set(1, 18);
        manager.totalPoints.set(0, 45);
        manager.totalPoints.set(1, 999);

        const summary = manager.getMapAdvanceTitleSummary(players);

        expect(summary.get(0)?.some((entry) => entry.key === 'mvp')).toBe(true);
        expect(summary.get(1)?.some((entry) => entry.key === 'mvp')).not.toBe(true);
    });
});

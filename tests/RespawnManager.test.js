...
describe('RespawnManager', () => {
    ...
    it('should queue a death and notify ScoreManager', () => {
        ...
        expect(mockScoreManager.recordDeath).toHaveBeenCalledWith(mockPlayer, null, null);
        expect(respawnManager.queue.length).toBe(1);
    });

    it('should forward trap type statistics to ScoreManager', () => {
        respawnManager.triggerDeath(mockPlayer, 'TRAP', 1, 'LASER');

        expect(mockScoreManager.recordDeath).toHaveBeenCalledWith(
            mockPlayer,
            1,
            'LASER',
        );
